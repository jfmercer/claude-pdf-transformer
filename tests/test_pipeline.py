...
@pytest.fixture
def compression_effective(monkeypatch):
    """Pretend Ghostscript shrinks any file to a tiny 1-page PDF."""

    def fake(src, dest, preset, gs_path):
...
@pytest.fixture
def compression_ineffective(monkeypatch):
...
def all_output_page_indices(outputs):
    indices = []
...
def test_compliant_pdf_copied_byte_identical(input_dir, output_dir):
...
def test_page_split_balanced_order_preserved(input_dir, output_dir):
...
def test_oversized_pdf_compressed_keeps_name(input_dir, output_dir, compression_effective):
...
def test_size_split_fallback_when_compression_insufficient(
    input_dir, output_dir, compression_ineffective
):
...
def test_single_page_over_limit_fails_without_output(
    ...
    assert "single page" in summary.failed[0].error
    assert list(output_dir.iterdir()) == []


def test_mixed_directory_bad_files_skipped(input_dir, output_dir):
...
def test_missing_ghostscript_raises_when_compression_needed(input_dir, output_dir, monkeypatch):
...
@gs_required
def test_real_ghostscript_end_to_end_compression(input_dir, output_dir):
